...
static mbedtls_aes_context titlekeyctx[3][2];

__attribute__((constructor))
void SetupCommonKeys(void) {
	for (int i = 0; i < 3; i++) {
		mbedtls_aes_setkey_dec(&titlekeyctx[i][0], CommonKeys[i], 128);
		mbedtls_aes_setkey_enc(&titlekeyctx[i][1], CommonKeys[i], 128);
	}
}
...
void GetTitleKey(tik* p_tik, aeskey out) {
	...
	mbedtls_aes_crypt_cbc(&titlekeyctx[commonKeyIndex][0], MBEDTLS_AES_DECRYPT, sizeof(aeskey),
...
void ChangeCommonKey(tik* p_tik, uint8_t index) {
	...
	mbedtls_aes_crypt_cbc(&titlekeyctx[index][1], MBEDTLS_AES_ENCRYPT, sizeof(aeskey),
